...
if language == "中文 (Chinese)":
    ...
    ui = {
        ...
        "button": "Run AI Diagnostic Analysis",
        "output_header": "🤖 AI Real-Time Diagnostic Insights",
        "error_empty": "Please provide either symptoms text or an image to analyze.",
        "analyzing": "Analyzing against global & local medical databases...",
        ...
        "disclaimer": "⚠️ Note: This is an AI clinical decision support tool. Final signature required by a licensed physician."
    }
    db_file = "dermatology_core_data_EN.json"
    default_disease_key = "atopic_dermatitis"
    acne_key = "acne_vulgaris"
    acne_keywords = ["acne", "pimple", "breakout"]

# 3. Load the corresponding Local Knowledge Base
...
with col1:
    ...
    symptoms = st.text_area(ui["symptoms_label"], placeholder=ui["symptoms_placeholder"])
    ...
    if analyze_btn:
        if not symptoms and not uploaded_image:
            ...
            with st.spinner(ui["analyzing"]):
                time.sleep(1.2) # Fast, simulated AI thinking time
                
                # Dynamic "Mock" AI Logic using the new JSON structure
                matched_disease_key = default_disease_key
                
                # Switch to Acne if keywords are detected
                if symptoms:
                    if any(keyword in symptoms.lower() for keyword in acne_keywords):
                        matched_disease_key = acne_key
                ...
                # The data structure has different keys depending on language, handle safely
                ...
                # Loop through the robust prescription data
                prescriptions = matched_data.get("处方", matched_data.get("prescriptions", {}))
                
                # Grab the first category of prescriptions (e.g., 'mild' or 'topical')
                    ...
                    for item in drugs:
                        with st.container(border=True):
                            if "drug" in item or "药品" in item:
                                ...
                                # Use safe dict.get() for varying fields between EN and CN
                                ...
                                # For surgical or special non-drug interventions
